...
public class Fish extends JLabel { //Timer本身就會產生一個thread，不用再implements Runnable
		...
		private ImageIcon[][] fishImg = {{new ImageIcon(getClass().getResource("fish_1.png")),new ImageIcon(getClass().getResource("fish_3.png")),new ImageIcon(getClass().getResource("fish_5.png"))},
					{new ImageIcon(getClass().getResource("fish_2.png")),new ImageIcon(getClass().getResource("fish_4.png")),new ImageIcon(getClass().getResource("fish_6.png"))}};//二元陣列,第一層是往右的魚,第二層是往左的魚
		...
		public void getBiger(){ //魚吃到魚餌，變大
			...
			fish_width = this.getIcon().getIconWidth(); //魚的寬度
			fish_height = this.getIcon().getIconHeight(); //魚的高度
			Fish.this.setBounds(fish_X,fish_Y,fish_width,fish_height); //設定魚的位置,長寬
		}
		...
		public void changeFishDirect(){ //改變魚的方向，遠離魚餌
			if(rightFlag == false){ //原本往左
				rightFlag = true; //改成往右
				fish_direct = 0;
				//改變魚的方向，重新存一次
				ImageIcon imageIcon = fishImg[fish_direct][fish_type]; //產生某種魚
