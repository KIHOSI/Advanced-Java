...
public class DSHW1_2B_103403530 {
	...
	public static double Calculate(double num1,char operator,double num2){ //數學計算
		double result = 0.0;
		if(operator == '+') //加法
		{
			result = num1 + num2;
...
/********************************中序轉後序*****************************************/
		...
		String transRes,transPos; //儲存從char[]格式轉到String格式的為了之後列印用
		System.out.printf("%-30s\t%-30s\t%-30s","Input Buffer","Operator Stack","Output String"); //列印標題
		System.out.println(); //格式化列印，使表格對齊
		while(res.length != 0) //直到res裡沒有元素才停止
		{
			transRes = String.valueOf(res); //transRes儲存轉成String格式的res
			transPos = String.valueOf(posfix); //transPos儲存轉成String
			//印出內容
...
/**********************************計算後序以求值***********************************/
		Stack<Double> st2 = new Stack<Double>(); //創造一個新的Double格式的stack，才可存兩位數以上的整數及浮點數
		for(int k=0;k<posfix.length;k++)
		{
			if((posfix[k] == '+')||(posfix[k] == '-') || (posfix[k] == '*') || (posfix[k] == '/')) //如果為運算子
			{
				double operand2 = st2.pop(); //operand2儲存st最上面的值
				double operand1 = st2.pop(); //operand1儲存第二個值
				char operator = posfix[k]; //operator儲存運算子
				double value = Calculate(operand1,operator,operand2); //value儲存計算結果(double格式)
				st2.push(value); //將結果丟進st2裡
			}
			else //如果為運算元
			{
				int operand = posfix[k]-48; //posfix[i]為char格式，轉成int會變成ASCII，減掉48就會得正確的值
				double transOper = (double)operand; //將正確的值轉成double格式
				st2.push(transOper); //將值丟入st2
			}
		}
		double answer = st2.pop(); //answer儲存最終答案
